...
import ru.netology.nework007.databinding.FragmentRegistrationBinding
import ru.netology.nework007.error.UnknownError
import ru.netology.nework007.media.MediaUpload
...
//println("viewModel.authState ${viewModel.authState.value}")
        with(binding) {
            registerButton.setOnClickListener {
                if (signUpLogin.text?.isEmpty() == true
                    || signUpPassword.text?.isEmpty() == true
                    || confirmPassword.text?.isEmpty() == true
                    || signUpName.text?.isEmpty() == true
                ) showBar(R.string.empty_field.toString())
                else {
                    if (confirmPassword.text?.contentEquals(signUpPassword.text) == false) {
                        showBar(R.string.not_confirm.toString())
                    } else {
                        if (upload != null) {
                            ...
                            pressBtn = true
                        } else showBar(R.string.missing_photo.toString())
